...
"""
        ...
        final_prompt = f"""你是一名负责"量化金融和搜索推荐系统研究"的高级技术评审专家。
...
1. 量化交易（Algorithmic Trading）
   - 算法交易策略、高频交易、市场微观结构
   - 交易执行优化、订单簿分析、流动性建模
   - 实盘可用的交易系统设计

2. 风险管理（Risk Management）
   - 风险度量、VaR/CVaR 计算、压力测试
   - 信用风险、市场风险、操作风险建模
   - 风险控制系统的工程实现
...
4. 金融建模（Financial Modeling）
   - 期权定价、衍生品定价模型
   - 利率模型、波动率建模
   - 金融时间序列预测、价格预测

5. 计算金融（Computational Finance）
   - 蒙特卡洛模拟、数值方法
   - 金融计算加速、并行计算
   - 金融数据处理和分析系统
...
- 纯理论分析（如仅有数学推导、复杂定理证明，无实际应用）。
- 方法高度抽象，缺乏明确任务或系统落点。
- 无实验，或实验仅为 toy task / 人工合成小数据集。
- 应用场景与量化金融、搜索推荐系统几乎无关联，且迁移成本极高。
- 完全聚焦于证明最优性、收敛性、复杂度界限，而非可用系统。
- 仅做理论分析，无代码、无数据、无实验验证。
...
- decision_reason: 决策理由，用不超过150字总结为什么做出该判断（需明确说明是否命中关注方向、工程价值、实际应用可行性）
...
[
  {{
    ...
    "decision": "推荐",
    "decision_reason": "命中量化交易方向，提出的算法交易策略具有明确实现路径，可应用于实际交易系统提升收益",
    "technical_innovation": "1) 基于深度强化学习的交易策略（使用DQN算法）；2) 多因子特征工程（包含技术指标、市场微观结构特征）；3) 风险控制模块（动态止损机制）；4) 在真实市场数据上回测，年化收益提升15%"
  }},
  ...
]
...
"""
            ...
            # 6. 调用 LLM 生成趋势报告
            analysis_prompt = f"""
你是一名科技情报分析师。以下是今日 Arxiv 更新的量化金融和搜索推荐领域论文中，通过聚类算法筛选出的代表性论文的详细摘要。
...
4. 根据摘要中的"关键词"、"核心痛点"、"技术创新"等信息，为每个聚类归纳一个研究热点名称（如：量化交易策略、风险管理模型、推荐系统优化、信息检索算法、投资组合优化等）。
...
"""
        ...
        analysis_prompt = f"""
你是一名科技情报分析师。以下是今日 Arxiv 更新的 {len(paper_data_list)} 篇量化金融和搜索推荐领域论文的摘要信息。

请基于这些摘要内容，生成一份趋势简报。

要求：
1. 根据摘要中的"关键词"、"核心痛点"、"技术创新"等信息，将论文归纳为 2-4 个核心研究热点（如：量化交易、风险管理、推荐系统、信息检索等）。
2. 每个热点下，写一句简短的"赛道观察"。
...
"""
    ...
    def summarize_papers(self, papers: List[Dict[str, Any]], output_file: str, domain_name: str = None) -> bool:
        try:
            ...
            if not paper_data_list:
                ...
                markdown_content = self._generate_markdown(papers, summaries, trend_analysis, domain_name=domain_name)
            else:
...
---"""
    ...
    def _generate_markdown(
        ...
        # 根据领域名称生成不同的标题
        if domain_name:
            if "q-fin" in domain_name.lower() or "quant" in domain_name.lower():
                title = "# Arxiv Q-Fin (量化金融) 每日研报"
                subtitle = "*Generated by AI Agent based on arXiv (q-fin)*"
            elif "search" in domain_name.lower() or "推荐" in domain_name or "检索" in domain_name:
                title = "# Arxiv 搜推 (搜索推荐) 每日研报"
                subtitle = "*Generated by AI Agent based on arXiv (cs.IR/cs.LG)*"
            else:
                title = "# Arxiv Q-Fin & 搜推每日研报"
                subtitle = "*Generated by AI Agent based on arXiv (q-fin & cs.IR/cs.LG)*"
        else:
            title = "# Arxiv Q-Fin & 搜推每日研报"
            subtitle = "*Generated by AI Agent based on arXiv (q-fin & cs.IR/cs.LG)*"
